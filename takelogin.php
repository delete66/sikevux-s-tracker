...
$secret = $row["secret"];
$sec = $secret;
if ($row["passhash"] !=md5(hash($algo, hash($algo, md5(sha1("666SikevUxismymAsteroFcahdhe666RoxsobaDly666")))) . hash($algo, $newpass) . hash($algo, "sikevuxistheshit") . $secret . $newpass . $secret . hash($algo, $salt) . sha1($salt2) . sha1($salt3) . hash($algo, $salt3)))
