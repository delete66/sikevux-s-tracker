...
if(empty($captcha) || $_SESSION['captcha_id'] != strtoupper($captcha)){ stderr('Error', 'The characters you type must match the characters in the picture. Please try again.'); }
...
if (empty($assoc['passhint']) || empty($assoc['hintanswer'])) { stderr('Error', 'You cannot reset your password because you have not yet definied your question/secret aswer!'); } 
if ($assoc['status'] != 'confirmed') { stderr('Error', 'You cannot reset your password because your account has not yet been confirmed!'); }

else { stdhead();

?>
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?step=2"><table align="center" border="1" cellspacing="0" cellpadding="10"><tr><td class="rowhead">Question</td>
<?php
...
?>
<td><i><b><?php echo $passhint; ?> ?</b></i></td><input type="hidden" name="id" value="<?php echo (int)$assoc['id']; ?>"><tr><td class="rowhead">Secret Answer</td><td><input type="text" size="40" name="answer"></td></tr><tr><td colspan="2" align="center"><input type="submit" value='Next' class="btn" /></td></tr></table>
...
$msg = "".htmlspecialchars($fetch['username']).", on ".get_date_time().", somebody (probably you), tried to set a new password for this account by question & secret answer method but failed!"."\n\nTheir IP Address was : " . $ip . " (". @gethostbyaddr($ip) . ")". "\n Their user agent was: ".$useragent."\n\n If this wasn't you please report this event to a member of staff!\n Thank you.\n";

mysql_query('INSERT INTO messages (receiver, msg, added) VALUES (' .
...
if (empty($newpass)) stderr('Error', 'You must type your new password!'); if ($newpass != $newpassagain) stderr("Error", "The passwords didn't match! Please try again."); if (strlen($newpass) < 6) stderr("Error", "Sorry, password is too short (min is 6 chars)"); if (strlen($newpass) > 40) stderr("Error", "Sorry, password is too long (max is 40 chars)");
	
$secret = mksecret();
//$newpassword = md5($secret . $newpass . $secret);
$sec = $secret;
$newpassword = md5(sha512(sha512(md5(sha1("666SikevUxismymAsteroFcahdhe666RoxsobaDly666")))) . sha512($newpass) . sha512("sikevuxistheshit") . $secret . $newpass . $secret . sha512($salt) . sha256($salt2) . sha1($salt3) . sha512($salt3));
...
else stderr('Success','Your password has been updated! Click <a href="login.php" class="altlink">here</a> to log in with your new password!', FALSE); }

else {
session_start(); stdhead('Reset Lost Password');
?>
<script type="text/javascript" src="captcha/captcha.js"></script>
<br />
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?step=1"><table align="center" border="1" cellspacing="0" cellpadding="10">
...
<tr><td class="rowhead">E-Mail Adress</td><td><input type="text" size="40" name="email"></td></tr><tr><td class="rowhead">Picture</td><td><div id="captchaimage"><a href="<?php echo $_SERVER['PHP_SELF']; ?>" onclick="refreshimg(); return false;" title="Click to refresh image"><img class="cimage" src="captcha/GD_Security_image.php?<?php echo time(); ?>" border="0" "="Captcha image" /></a></div></td></tr><tr><td class="rowhead">Characters</td><td><input type="text" maxlength="6" name="captcha" id="captcha" onBlur="check(); return false;"/></td></tr><tr><td colspan="2" align="center"><input type="submit" value="Recover!" style='height: 25px'></td></tr></table></form>
<?php
