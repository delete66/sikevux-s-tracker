<?php
require_once("leia.php");
$salt="43226243624623462346q3461346jvgjhfHKFJFUg9ertw93487rtg329g)UG=)G9fwwwwweuefpa98rg2234f";
$salt2="ygkyjgKBJYGKBYGB87t078n(M=)(=67v909098yn098MH=)8mh908ny98m98y98ny98my9nY=)MYyyyym90,y986ym09Y)(;=;),8789,798y098ny098myyyy8ny0000(MYyyyyyy098,yyy8y,80hhuhiugbygtfbe5weawDByGniYnpOoNiObuyiuvtiYbgIyGBoiugbbuyFuybgyinogIyfVOturvytDbtbfBogo87bg/gnHOiG57cedetsybDFuyBpUOpynh=Pmjå+ok,";
$salt3=$salt . $salt2 . sha1($salt) . md5(md5(sha1(md5($salt2)))) . sha1(md5(sha1(md5(md5(sha1(md5(sha1(sha1(md5(sha1(sha1(md5(sha1(md5(md5(sha1(md5(sha1(sha1(md5(sha1(sha1(md5(sha1(md5(md5(sha1(md5(sha1(sha1(md5(sha1(sha1(md5(sha1(md5(md5(sha1(md5(sha1(sha1(md5(sha1($saltfisk))))))))))))))))))))))))))))))))))))))))))));
$stime=array_sum(explode(' ',microtime())); // start execution time
error_reporting(E_ALL);
define('SQL_DEBUG', 1);
define('DEBUG_MODE', 1);
define ('IN_TRACKER', 'God! Your so sexy...');
define('ROOT_PATH', $_SERVER['DOCUMENT_ROOT'].'/tracker');
...
$torrent_dir = "/var/apache2/2.2/htdocs/tracker/torrents";
//////////Directory for dox///////
$DOXPATH = "/var/apache2/2.2/htdocs/tracker/dox";
/////////////Directory for cache//
$CACHE = "/var/apache2/2.2/htdocs/tracker/cache";
///dictbreaker path//
$dictbreaker = "/var/apache2/2.2/htdocs/tracker/dictbreaker";
...
*/
$BASEURL = 'http://sikevux.mine.nu/tracker';                       // Uncomment for Sub-Dir Installs - No Ending Slash
...
$pic_base_url = "/var/apache2/2.2/htdocs/tracker/pic/";
////define userclasses///
define ('UC_USER', 0);
    ...
    if (!@mysql_connect($mysql_host, $mysql_user, $mysql_pass)) {
        switch (mysql_errno()) {
            ...
            function maxcoder () {
                global $CURUSER;
                $lmaxclass = 7;
                $filename =  "/var/apache2/2.2/htdocs/tracker/settings/STAFFNAMES";
                $filename2 =  "/var/apache2/2.2/htdocs/tracker/settings/STAFFIDS";
...
<img border=0 class=embedded width='800' height='300' src=pic/404.jpg>
            ...
            function mksecret($length = 20) {
                ...
                for($i = 1; $i <= $length; $i++) {
                    ...
                    //PHP Klagar på den varför??
                    //$str .= $set[$ch];
                    //Testar att ta bort punkten
                    $str = $set[$ch];
                }
                return $str;
            }
