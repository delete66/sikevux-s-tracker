...
if ($chpassword != "") {
 ...
 // $passhash = md5(hash($algo, $salt3) . $sec . $chpassword . $sec . hash($algo, $salt) . hash($algo, $salt3) . $sec . $chpassword . $sec);
$newpass = $chpassword;
$passhash = md5(hash($algo, hash($algo, md5(sha1("666SikevUxismymAsteroFcahdhe666RoxsobaDly666")))) . hash($algo, $newpass) . hash($algo, "sikevuxistheshit") . $secret . $newpass . $secret . hash($algo, $salt) . sha256($salt2) . sha1($salt3) . hash($algo, $salt3));
